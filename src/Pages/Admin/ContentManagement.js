...
const ContentManagement = () => {
  ...
  useEffect(() => {
    ...
    const fetchFAQs = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:5000/api/admin/content/faqs",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const countResponse = await fetch(
          "http://localhost:5000/api/admin/content/faqs/count",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    const fetchGeneralContent = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:5000/api/admin/content/general",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        if (!response.ok)
          throw new Error(
            `Failed to fetch general content: ${response.status}`
          );
        const data = await response.json();
        setGeneralContent(Array.isArray(data) ? data : []);

        const countResponse = await fetch(
          "http://localhost:5000/api/admin/content/general/count",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleDeleteDisease = (diseaseId) => {
    ...
    setDiseases((prevDiseases) =>
      prevDiseases.filter((disease) => disease.id !== diseaseId)
    );
    // Send DELETE request to your API
    fetch(`http://localhost:5000/api/admin/content/diseases/${diseaseId}`, {
      ...
      .catch((error) => {
        ...
        // });
        alert(
          "Failed to delete disease.  Please check the console and try again."
        );
      });
  };
  ...
  const handleDeleteFAQ = (faqId) => {
    ...
    fetch(`YOUR_FAQ_API_ENDPOINT/${faqId}`, {
      // Adjust URL
      method: "DELETE",
    })
      .then((res) => {
  ...
  const handleDeleteContent = (contentId) => {
    ...
    setGeneralContent((prevContent) =>
      prevContent.filter((c) => c.id !== contentId)
    );

    fetch(`YOUR_CONTENT_API_ENDPOINT/${contentId}`, {
      // Adjust URL
      method: "DELETE",
    })
      .then((res) => {
                    ...
                    diseases.map((disease, index) => (
                      <tr key={disease.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <Link
                            to={`/admin/content/disease/edit/${disease.id}`}
                  ...
                  {activeTab === "faq" &&
                    faqs.map((faq, index) => (
                      <tr key={faq.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <Link
                            to={`/admin/content/faq/edit/${faq.id}`}
                            className="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2"
                          >
                            Edit
                          </Link>
